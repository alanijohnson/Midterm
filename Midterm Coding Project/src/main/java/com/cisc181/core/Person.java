...
public abstract class Person implements java.io.Serializable {
	...
	public void setDOB(Date DOB) throws PersonException {
		...
		final double msecPerYear = (1000.0 * 60 * 60 * 24 * 365);
		if ((currentdate.getTime() - DOB.getTime()) > (100 * msecPerYear)) {
	...
	public void setPhone(String newPhone_number) throws PersonException {
		Pattern pattern;
		Matcher matcher;
		pattern = Pattern.compile(phone_number_pattern); // sets the pattern
		matcher = pattern.matcher(newPhone_number);
		if (!(matcher.matches())) {
			throw new PersonException(this);
		} else {
